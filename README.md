...
En Cloudflare, zona `fibot.ar`: el **apex** va con **registros A** a las cuatro
IP de GitHub Pages (`185.199.108.153`, `.109.153`, `.110.153`, `.111.153`) — un
CNAME no es válido en la raíz de una zona. `www` resuelve a las mismas cuatro;
GitHub redirige al dominio que figure en `CNAME`, que es el apex.

Todo **sin proxy** (nube gris): con el proxy naranja GitHub no puede emitir el
certificado. Para comprobarlo alcanza con resolver el dominio — si contesta
`185.199.*` está derecho a Pages; `104.*` o `172.67.*` es Cloudflare proxeando.
